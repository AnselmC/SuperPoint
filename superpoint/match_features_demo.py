...
def extract_superpoint_keypoints_and_descriptors(
    ...
    keypoints = [cv2.KeyPoint(p[1], p[0], 1) for p in keypoints.astype(float)]
...
def compute_homography(matched_kp1, matched_kp2):
    ...
    H, inliers = cv2.findHomography(
        matched_pts1[:, [1, 0]], matched_pts2[:, [1, 0]], cv2.RANSAC
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Compute the homography \
    ...
    with tf.Session(graph=graph) as sess:
        tf.saved_model.loader.load(
            sess, [tf.saved_model.tag_constants.SERVING], str(weights_dir)
        )
